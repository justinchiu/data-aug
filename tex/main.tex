...
The above objective is sensitive to the reference model $p$,
...
\begin{equation}
    \begin{aligned}
    \maximize_\alpha & \max_q \sum_i \sum_j \alpha_{i,j}  \log q(\hat{y}_{i,j} \mid x_i)\\
...
We can also resort to approximations of bilevel optimization.
The inner maximization of $q$ couples all the elements of $\alpha$ together.
One example may contain information that is useful for modeling other examples as well.
However, if we make the simplifying assumption that this is not the case,
then, treating $q$ as a constant, maximizing $\alpha$ would entail
picking the highest scoring $\hat{y}$ under $q$.
This entails training $q$ on the new dataset,
and retaining the highest scoring examples under the new model.
We refer to this as a greedy approximation.
